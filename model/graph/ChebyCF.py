...
class ChebyCF(GraphRecommender):
    ...
    def train(self):
        ...
        epoch = 0
        # x_start = TorchGraphInterface.convert_sparse_mat_to_tensor(self.data.interaction_mat).to_dense()
        model.fit(self.data.interaction_mat)
            
        with torch.no_grad():
            total_batch = math.ceil(self.data.user_num/self.batch_size)
            self.prediction = []
            for batch in range(total_batch):
                batch_x_start = TorchGraphInterface.convert_sparse_mat_to_tensor(self.data.interaction_mat[batch*self.batch_size:(batch+1)*self.batch_size]).cuda().to_dense()
                batch_prediction = model.full_predict(batch_x_start).cpu().numpy()
                self.prediction.append(batch_prediction)
...
class ChebyFilter(nn.Module):
    ...
    def fit(self, inter):
        # Laplacian_tilde
        self.laplacian = Laplacian(inter).cuda() # shape (num_items, num_items)
        
        # Chebyshev Nodes and Target Transfer Function Values
        cheby_nodes = torch.arange(1, (self.order+1)+1) # open bracket + next stage for prediction
        cheby_nodes = torch.cos(((self.order+1) + 0.5 - cheby_nodes) / (self.order+1) * math.pi).cuda() 
        target = self.plateau().cuda()
        # Chebyshev Interpolation Coefficients
        coeffs = self.cheby(x=cheby_nodes, init=target).sum(dim=1) * (2/(self.order+1))
        coeffs[0] /= 2 # a_k = 1 for k = 0, and a_k = 2 otherwise 
...
class IdealFilter(nn.Module):
    ...
    def fit(self, inter):
        norm_inter = get_norm_inter(inter)
        _, _, vt = svds(norm_inter, which='LM', k=self.threshold) # largest magnitude for Adj.
        ideal_pass = torch.tensor(vt.T.copy()).cuda()
...
class DegreeNorm(nn.Module):
    ...
    def fit(self, inter):
        ...
        pre_norm = item_degree.clamp(min=1).pow(-self.power).cuda()
        pst_norm = item_degree.clamp(min=1).pow(+self.power).cuda()
        pre_norm[zero_mask], pst_norm[zero_mask] = 0, 0
        self.register_buffer('pre_normalize', pre_norm)  # (num_items,) # normalized
        self.register_buffer('post_normalize', pst_norm) # (num_items,) # reverse-normalized
...
class ChebyCF_Encoder(nn.Module):
    def __init__(self, data, emb_size, K, phi, eta, alpha, beta):
        ...
        self.alpha = alpha 
        self.beta = beta
        self.cheby_filter = ChebyFilter(K, phi).cuda()
        self.ideal = IdealFilter(self.eta, self.alpha).cuda() if self.eta>0 and self.alpha > 0 else None
        self.norm = DegreeNorm(self.beta).cuda() if self.beta > 0 else None
    
    def fit(self, inter):
        self.cheby_filter.fit(inter)
        if self.ideal:
    ...
    def forward(self, signal):
        ...
        output = self.cheby_filter.forward(signal)
        if self.ideal:
